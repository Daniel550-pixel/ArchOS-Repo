...
async def append(entity_id, tenant, actor, patch, event_type="STATE_CHANGED",
                 reality="OBSERVED", confidence=1.0, provenance=""):
    event_record = {
        "id": str(uuid.uuid4()),
    ...
    try:
        await fabric.publish("WORLD_MODEL_EVENT", {
            "entity_id": str(entity_id),
            "event_type": event_type,
            "reality": reality,
            "confidence": confidence,
...
def history(entity_id: str) -> list:
    return [
        ...
        for e in _MEM_EVENTS if e["entity_id"] == entity_id
    ]

def as_of(entity_id: str, ts_point: datetime) -> dict:
    events = [e for e in _MEM_EVENTS if e["entity_id"] == entity_id and e["ts"] <= ts_point]
    state = {}
    for ev in sorted(events, key=lambda x: x["ts"]):
        if isinstance(ev.get("patch"), dict):
            state.update(ev["patch"])
