...
def _user(req: Request):
    ...
    if auth.startswith("Bearer "):
        ...
        try:
            if jwt:
                payload = jwt.decode(tok, secret("JWT_SECRET", "archos_sovereign_jwt_secret_key_2026_qkd"), algorithms=["HS256"])
                sub = payload.get("sub", "operator")
        except Exception:
            pass
    if sub not in USERS:
        USERS[sub] = {
...
async def unlock(req: Request, body: dict):
    ...
    try:
        import jwt as _jwt
        unlock_tok = _jwt.encode(token_payload, secret("JWT_SECRET", "archos_sovereign_jwt_secret_key_2026_qkd"), "HS256")
    except Exception:
        unlock_tok = "keysmith_unlocked_" + base64.urlsafe_b64encode(os.urandom(16)).decode().rstrip("=")
