...
def calculate_chain_digest(entry: EvidenceEntry, previous_digest: str | None) -> str:
    return hashlib.sha256(canonical_chain_payload(entry, previous_digest)).hexdigest()
...
class PostgresEvidenceStore:
    ...
    async def corroborating(self, session: AsyncSession, task_id: str, agent_id: str) -> list[dict[str, Any]]:
        rows = (await session.scalars(select(EvidenceRecordModel).where(EvidenceRecordModel.task_id == task_id, EvidenceRecordModel.agent_id != agent_id).order_by(EvidenceRecordModel.created_at.asc(), EvidenceRecordModel.id.asc()))).all()
    ...
    async def history(self, session: AsyncSession, task_id: str | None = None) -> list[dict[str, Any]]:
        ...
        return [{"entry_id": r.entry_id, "task_id": r.task_id, "agent_id": r.agent_id, "source": r.source, "claim": r.claim, "evidence": r.evidence, "confidence": r.confidence, "reality": r.reality, "created_at": r.created_at.isoformat(), "digest": r.digest, "previous_digest": r.previous_digest, "chain_digest": r.chain_digest} for r in rows]

    async def verify_chain(self, session: AsyncSession) -> dict[str, Any]:
        rows = (await session.scalars(select(EvidenceRecordModel).order_by(EvidenceRecordModel.id.asc()))).all()
        previous: str | None = None
        failures: list[dict[str, Any]] = []
        for row in rows:
            entry = EvidenceEntry(entry_id=row.entry_id, task_id=row.task_id, agent_id=row.agent_id, source=row.source, claim=row.claim, evidence=tuple(row.evidence or []), confidence=row.confidence, reality=RealityLevel(row.reality), created_at=row.created_at.isoformat(), digest=row.digest)
