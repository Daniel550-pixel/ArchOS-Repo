...
def _article_out(article: Article) -> dict:
    return {
        ...
        "importance_factors": article.importance_factors,
        "confidence_score": article.confidence_score,
        "confidence_reason": article.confidence_reason,
        "detected_emirate": article.detected_emirate,
...
@router.get("/articles")
async def list_world_model_articles(
...
@router.get("/events")
async def list_world_model_events(
    limit: int = Query(100, ge=1, le=500),
...
@router.get("/entities")
async def list_world_model_entities(
    ...
    if emirate and emirate.upper() != "ALL":
        stmt = stmt.where(Entity.emirate.ilike(f"%{emirate}%"))
    result = await db.execute(stmt)
...
async def list_world_model_locations(
    limit: int = Query(500, ge=1, le=2000),
    emirate: Optional[str] = None,
    db: AsyncSession = Depends(get_db),
):
    ...
    if emirate and emirate.upper() != "ALL":
        stmt = stmt.where(Location.emirate.ilike(f"%{emirate}%"))
    result = await db.execute(stmt)
