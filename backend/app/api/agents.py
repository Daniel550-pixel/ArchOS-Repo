...
This router exposes registration and task-submission primitives without exposing
internal agent implementations. Execution remains inside the governed runtime.
"""
...
# Process-local registry is intentionally small for the first public contract.
# Persistent/distributed registry belongs in a later runtime milestone.
...
async def register_agent(request: AgentRegistration):
    ...
    record.update({"agent_id": agent_id, "status": "REGISTERED", "registered_at": now})
...
async def submit_agent_task(agent_id: str, request: AgentTaskRequest):
    ...
    required = set(cap.value for cap in request.required_capabilities)
