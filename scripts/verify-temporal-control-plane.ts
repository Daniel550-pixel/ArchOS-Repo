...
async function main(){
 ...
 const session=sessionIntelligence.start('temporal verification'); const root='trace-root',child='trace-child';
 ...
 const replay=missionReplay.getSession(session);assert(replay?.integrityValid);const branch=temporalControlPlane.branchFrom(session,1);assert(branch);assert.equal(branch.sourceTraceId,child);assert.equal(temporalControlPlane.appendBranchCommand(branch.id,{type:'WHAT_IF',value:'test'})?.parentTraceId,child);assert.equal(temporalControlPlane.beginSimulation(branch.id),true);assert.equal(temporalControlPlane.getBranch(branch.id)?.status,'SIMULATING');assert.equal(temporalControlPlane.completeSimulation(branch.id,'COMPLETED'),true);assert.equal(temporalControlPlane.getBranch(branch.id)?.status,'COMPLETED');
 console.log('Temporal control plane verification: PASS'); sessionIntelligence.shutdown();executionTrace.shutdown();ultronEventBus.clear();
}
