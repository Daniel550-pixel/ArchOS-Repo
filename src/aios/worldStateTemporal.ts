...
export const worldStateTemporal = {
  reconstruct(sessionId: string, frameIndex: number): WorldStateSnapshot | null {
    ...
    return { id: createAIOSTraceId(), sessionId, frameIndex: index, traceId: current?.traceId ?? null, timestamp: current?.timestamp ?? replay.session.startedAt, entities: clone(entities), entityCount: Object.keys(entities).length, provenanceValid, hypothetical: false };
  },
