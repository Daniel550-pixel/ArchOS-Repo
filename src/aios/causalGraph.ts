...
export const causalGraph={
 ...
 lineage(sessionId:string,traceId:string){const g=this.build(sessionId);if(!g)return[];const byId=new Map(g.nodes.map(n=>[n.traceId,n])),out:CausalGraphNode[]=[],seen=new Set<string>();let n=byId.get(traceId);while(n&&!seen.has(n.traceId)){seen.add(n.traceId);out.unshift(n);n=n.parentTraceId?byId.get(n.parentTraceId):undefined;}return out;},
