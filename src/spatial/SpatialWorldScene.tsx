...
import { AdditiveBlending, Color, Group, LineBasicMaterial, Mesh, Points, PointsMaterial, Vector3 } from 'three';
...
const MODULE_COUNT = SPATIAL_MODULES.length;
...
function Core() {
  ...
  useFrame(({ clock }, dt) => {
    ...
    const pulse = 1 + Math.sin(clock.elapsedTime * 1.7) * .025;
...
function AccretionDisk() {
  ...
  const points = useMemo(() => Array.from({ length: 2600 }, (_, i) => {
    ...
    const thickness = (Math.random() - .5) * (.055 + r * .035);
    const speed = .15 + 1.15 / Math.sqrt(r);
    const size = .006 + Math.random() * .018;
    return { a, r, thickness, speed, size, seed: i * 1.731 };
  }), []);
  useFrame(({ clock }, dt) => { ref.current.rotation.y += dt * .09; });
  return <group ref={ref} rotation={[.13, 0, .07]}>
    {points.map((p, i) => {
      const a = p.a + clocklessPhase(p.seed);
...
function clocklessPhase(seed: number): number { return Math.sin(seed) * .015; }

function RelativisticJets() {
  return <group>
    {[-1, 1].map((direction) => <group key={direction} position={[0, direction * .8, 0]}>
      <mesh scale={[.22, 3.8, .22]}><coneGeometry args={[1, 1, 20, 1, true]} rotation={[direction > 0 ? 0 : Math.PI, 0, 0]}/><meshBasicMaterial color="#6db7ff" transparent opacity={.11} blending={AdditiveBlending}/></mesh>
...
function ModuleNode({ module, onSelect }: { module: SpatialModule; onSelect?: (id: string) => void }) {
  ...
  useFrame(({ clock }) => {
    ...
    const p = modulePositionAtTime(module, t);
    group.current.position.lerp(p, .08);
    const activity = normalizedActivity(module, t);
    core.current.scale.setScalar(.7 + activity * .7);
  });
  return <group ref={group} position={modulePositionAtTime(module, 0)}>
    <mesh ref={core} onClick={(e) => { e.stopPropagation(); onSelect?.(module.id); }}>
      <sphereGeometry args={[.14 + module.activity * .045, 18, 18]}/>
      <meshBasicMaterial color={module.color} transparent opacity={.95} blending={AdditiveBlending}/>
    </mesh>
...
function CoreConnections() {
  const lineRefs = useRef<LineBasicMaterial[]>([]);
  ...
  useFrame(({ clock }) => {
    lineRefs.current.forEach((material, i) => { if (material) material.opacity = .08 + normalizedActivity(SPATIAL_MODULES[i], clock.elapsedTime) * .2; });
  });
  return <lineSegments>
    <bufferGeometry><bufferAttribute attach="attributes-position" args={[vertices, 3]}/></bufferGeometry>
    <lineBasicMaterial ref={(node) => { if (node) lineRefs.current = [node]; }} color="#8ed8ff" transparent opacity={.15} blending={AdditiveBlending}/>
  </lineSegments>;
}

function OrbitField() {
  return <>{[2.05, 2.65, 3.25, 3.85, 4.45, 5.05].map((r, i) => <mesh key={r} rotation={[Math.PI / 2, 0, i * .08]}>
    <torusGeometry args={[r, i % 2 ? .007 : .004, 6, 220]}/>
    <meshBasicMaterial color={i % 2 ? '#5b8dff' : '#ffb86b'} transparent opacity={.035 + i * .008} blending={AdditiveBlending}/>
  </mesh>)}</>;
}

function DeepField() {
  return <>
    <Sparkles count={2400} scale={[22, 11, 22]} size={1.15} speed={.12} opacity={.52} color="#b9d8ff"/>
    <Sparkles count={600} scale={[13, 6, 13]} size={2.2} speed={.24} opacity={.32} color="#ffcf9a"/>
  </>;
}
...
export function SpatialWorldScene({ onModuleSelect }: { onModuleSelect?: (id: string) => void }) {
  return <>
    ...
    <OrbitField/>
    <CoreConnections/>
    {SPATIAL_MODULES.map((module) => <ModuleNode key={module.id} module={module} onSelect={onModuleSelect}/>) }
    <EffectComposer multisampling={0}>
      <Bloom intensity={1.9} luminanceThreshold={.13} luminanceSmoothing={.5} mipmapBlur/>
