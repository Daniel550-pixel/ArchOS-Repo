...
export const UltronOneWorld: React.FC = () => {
  const [runtime,setRuntime]=useState<AIOSRuntimeState>(()=>aiosRuntime.getState()); const [selected,setSelected]=useState<Star|null>(null); const [query,setQuery]=useState(''); const [listening,setListening]=useState(false); const [speaking,setSpeaking]=useState(false); const [pointer,setPointer]=useState({x:0,y:0}); const inputRef=useRef<HTMLInputElement>(null);
  useEffect(()=>aiosRuntime.subscribe(setRuntime),[]);
  useEffect(()=>{const onKey=(e:KeyboardEvent)=>{if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='k'){e.preventDefault();inputRef.current?.focus();}if(e.key==='Escape'){setSelected(null);inputRef.current?.blur();}};window.addEventListener('keydown',onKey);return()=>window.removeEventListener('keydown',onKey);},[]);
  ...
  const toggleSpeech=()=>{if(speaking){speechService.stopSpeaking();setSpeaking(false);return;}speechService.speak(selected?`${selected.name}. ${selected.type}. World model entity.`:'ULTRON world model online.');setSpeaking(true);window.setTimeout(()=>setSpeaking(false),2200);};
  return <div className="one-world" onPointerMove={e=>{const r=e.currentTarget.getBoundingClientRect();setPointer({x:(e.clientX-r.left)/r.width-.5,y:(e.clientY-r.top)/r.height-.5});}} onPointerLeave={()=>setPointer({x:0,y:0})}>
    ...
    <div className="one-world-black-hole" aria-label="ULTRON AIOS intelligence core"><div className="black-hole-glow"/><div className="black-hole-accretion"/><div className="black-hole-event-horizon"/><div className="black-hole-center"/><div className="black-hole-label"><strong>ULTRON</strong><span>AIOS · WORLD MODEL</span></div></div>
