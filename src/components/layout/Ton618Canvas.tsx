  ...
  useEffect(() => {
    ...
    const drawJets = (cx: number, cy: number) => {
      ...
      for (const direction of [-1, 1]) {
        ...
        ctx.beginPath();
        ctx.moveTo(cx - widthPx, cy); ctx.lineTo(cx + widthPx, cy);
    ...
    const drawGlow = (cx: number, cy: number) => {
      ...
      g.addColorStop(0, 'rgba(255,245,220,0.20)'); g.addColorStop(0.16, 'rgba(255,165,80,0.12)');
      g.addColorStop(0.36, 'rgba(75,205,255,0.09)'); g.addColorStop(0.72, 'rgba(35,100,180,0.025)'); g.addColorStop(1, 'rgba(0,0,0,0)');
    ...
    const drawLensing = (cx: number, cy: number) => {
      for (let i = 0; i < 7; i++) {
        ...
        ctx.beginPath(); ctx.ellipse(cx, cy, radius, radius * 0.30, 0, 0, Math.PI * 2); ctx.stroke();
      }
    };

    const drawBlackHole = (cx: number, cy: number) => {
      const ring = ctx.createRadialGradient(cx, cy, 44, cx, cy, 64);
      ring.addColorStop(0, 'rgba(0,0,0,0)'); ring.addColorStop(0.55, 'rgba(255,235,180,0.95)');
      ring.addColorStop(0.72, 'rgba(130,235,255,0.35)'); ring.addColorStop(1, 'rgba(255,150,60,0)');
      ctx.fillStyle = ring; ctx.beginPath(); ctx.arc(cx, cy, 64, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(cx, cy, 48, 0, Math.PI * 2); ctx.fill();
      const inner = ctx.createRadialGradient(cx, cy, 0, cx, cy, 48);
      inner.addColorStop(0, '#000'); inner.addColorStop(0.82, '#000'); inner.addColorStop(1, 'rgba(20,15,10,0.98)');
      ctx.fillStyle = inner; ctx.beginPath(); ctx.arc(cx, cy, 48, 0, Math.PI * 2); ctx.fill();
    };

    const animate = () => {
      ctx.fillStyle = 'rgba(0,0,0,0.16)'; ctx.fillRect(0, 0, width, height);
      const cx = width / 2 + mouseX * 38; const cy = height / 2 + mouseY * 28;
      rotation += 0.0018; drawStars(); drawJets(cx, cy); drawGlow(cx, cy); drawDisk(cx, cy); drawLensing(cx, cy); drawBlackHole(cx, cy);
