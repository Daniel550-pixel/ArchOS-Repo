...
export function Ton618Canvas() {
  ...
  useEffect(() => {
    ...
    const stars: Star[] = Array.from({ length: 520 }, () => ({
      x: (Math.random() - 0.5) * 2,
      y: (Math.random() - 0.5) * 2,
      z: Math.random() * 2200 + 20,
      size: Math.random() * 1.8 + 0.15,
      brightness: Math.random() * 0.75 + 0.2,
      tint: Math.random(),
    }));

    const disk: DiskParticle[] = Array.from({ length: 2800 }, () => ({
      angle: Math.random() * Math.PI * 2,
      radius: 58 + Math.pow(Math.random(), 0.72) * 390,
      speed: 0.014 + Math.random() * 0.036,
      size: Math.random() * 1.8 + 0.35,
      brightness: Math.random() * 0.55 + 0.45,
      phase: Math.random() * Math.PI * 2,
    }));

    const jetParticles: JetParticle[] = Array.from({ length: 700 }, () => ({
      t: Math.random(),
      side: Math.random() > 0.5 ? 1 : -1,
      spread: Math.random(),
      speed: 0.025 + Math.random() * 0.065,
      size: Math.random() * 1.8 + 0.35,
    ...
    const drawStars = (dt: number) => {
      for (const star of stars) {
        ...
        const color = star.tint > 0.78 ? `rgba(150,205,255,${alpha})` : `rgba(225,242,255,${alpha})`;
        ctx.fillStyle = color;
        ctx.beginPath();
    ...
    const drawJets = (cx: number, cy: number) => {
      ...
      for (const side of [-1, 1]) {
        ...
        ctx.fillStyle = outer;
        ctx.beginPath();
        ctx.moveTo(cx - core * 4.5, cy);
        ctx.lineTo(cx + core * 4.5, cy);
        ctx.lineTo(cx + core * 0.8, cy + side * length);
        ctx.lineTo(cx - core * 0.8, cy + side * length);
    ...
    const drawBlackHole = (cx: number, cy: number) => {
      ...
      ctx.fillStyle = ring; ctx.beginPath(); ctx.arc(cx, cy, 72, 0, Math.PI * 2); ctx.fill();
      ctx.restore();
      ...
      ctx.fillStyle = shadow; ctx.beginPath(); ctx.arc(cx, cy, 58, 0, Math.PI * 2); ctx.fill();

      ctx.save();
      ctx.globalCompositeOperation = 'screen';
      const edge = ctx.createRadialGradient(cx, cy, 48, cx, cy, 62);
      edge.addColorStop(0, 'rgba(0,0,0,0)'); edge.addColorStop(0.65, 'rgba(255,245,205,0.65)'); edge.addColorStop(1, 'rgba(255,140,50,0)');
      ctx.fillStyle = edge; ctx.beginPath(); ctx.arc(cx, cy, 62, 0, Math.PI * 2); ctx.fill();
      ctx.restore();
    };
