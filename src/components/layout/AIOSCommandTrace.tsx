...
export const AIOSCommandTrace: React.FC<{ visible?: boolean }> = ({ visible = true }) => {
  ...
  const persistSession = (nextEntries: TraceEntry[]) => {
    ...
    setSessions((current) => {
      ...
      const next: StoredSession = existing ? { ...existing, updatedAt: now, entries: nextEntries } : { id: nextEntries[0].id.toString(), startedAt: nextEntries[0].timestamp, updatedAt: now, entries: nextEntries, title };
  ...
  useEffect(() => {
    ...
    const disposers = [
      ultronEventBus.on('input.command', ({ command, source }) => { add('input', `${command.type} · ${source}`); add('intent', 'Intent accepted by AIOS'); }),
  ...
  const clearHistory = () => { setSessions([]); setSelectedSession(null); try { window.localStorage.removeItem(STORAGE_KEY); } catch { /* storage is optional */ } };
  ...
  return <aside className="aios-command-trace" aria-live="polite" aria-label="AIOS command trace">
    <header className="aios-trace-header"><span><i /> {view === 'history' ? 'SESSION INTELLIGENCE' : 'COMMAND TRACE'}</span><div className="aios-trace-controls"><button type="button" className={view === 'live' ? 'is-active' : ''} onClick={() => { setView('live'); setSelectedSession(null); }}>LIVE</button><button type="button" className={view === 'history' ? 'is-active' : ''} onClick={() => setView('history')}>HISTORY {sessions.length ? `· ${sessions.length}` : ''}</button>{view === 'live' && <button type="button" onClick={clearLive}>CLEAR</button>}{view === 'history' && <button type="button" onClick={clearHistory}>PURGE</button>}</div></header>
    {view === 'history' && !selectedSession ? <div className="aios-trace-sessions">{sessions.map((session) => <button key={session.id} type="button" className="aios-trace-session" onClick={() => setSelectedSession(session.id)}><span className="aios-trace-session-status" /><span><strong>{session.title}</strong><small>{new Date(session.updatedAt).toLocaleString([], { hour12: false })} · {session.entries.length} events</small></span><b>→</b></button>)}</div> : <div className="aios-trace-stream">{displayEntries.map((entry) => <div key={entry.id} className={`aios-trace-entry is-${entry.status}`}><span className="aios-trace-time">{new Date(entry.timestamp).toLocaleTimeString([], { hour12: false })}</span><span className="aios-trace-marker" /><div className="aios-trace-copy"><strong>{entry.label}</strong><span>{entry.detail}</span></div></div>)}{view === 'history' && <button type="button" className="aios-trace-back" onClick={() => setSelectedSession(null)}>← BACK TO SESSION HISTORY</button>}</div>}
  </aside>;
};
