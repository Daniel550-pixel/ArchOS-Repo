 ...
 *
 * This is the model-agnostic experience layer. Models and providers are not
 * represented in the visual identity; they operate behind the command and
    ...
    <section className="archos-unified-spatial" aria-label="ArchOS unified spatial intelligence canvas">
      ...
      <header className="archos-spatial-header archos-surface">
        <div className="archos-spatial-brand">
          ...
          <div>
            ...
            <span>SPATIAL INTELLIGENCE OPERATING ENVIRONMENT</span>
          </div>
        </div>
        <nav className="archos-spatial-modes" aria-label="Spatial modes">
          {([['WORLD', Map], ['INTELLIGENCE', Network], ['SIMULATION', Play]] as const).map(([item, Icon]) => (
            <button key={item} type="button" className={mode === item ? 'is-active' : ''} onClick={() => { setMode(item); if (item === 'INTELLIGENCE') setShowFabric(true); }}>
              <Icon size={11} /> {item}
            </button>
          ))}
        </nav>
      ...
      <aside className="archos-spatial-rail archos-surface">
        ...
        <div className="archos-spatial-layer-list">
          {(['ALL', 'SKYLINE', 'MOBILITY', 'SENSORS', 'SUBSURFACE'] as ActiveLayer[]).map((layer) => (
            <button key={layer} className={activeLayer === layer ? 'is-active' : ''} onClick={() => setActiveLayer(layer)} type="button">
              <Layers3 size={13} /> {layer}
            </button>
          ))}
        </div>
        ...
        <div className="archos-camera-grid">
          {(['COMMAND', 'ORBIT', 'NADIR', 'STREET'] as const).map((preset) => (
            <button key={preset} className={cameraPreset === preset ? 'is-active' : ''} onClick={() => setCameraPreset(preset)} type="button">
              {preset}
            </button>
          ))}
        </div>
        ...
        <select value={lightingMode} onChange={(event) => setLightingMode(event.target.value as LightingMode)} aria-label="Rendering mode">
        ...
        <label className="archos-spatial-slider">
          ...
          <input type="range" min="0" max="1.5" step="0.05" value={sliceSeparation} onChange={(event) => setSliceSeparation(Number(event.target.value))} />
        </label>

        <div className="archos-spatial-runtime">
          <div><Activity size={11} /><span>RUNTIME</span><b>LIVE</b></div>
          <div><Cpu size={11} /><span>LOAD</span><b>37%</b></div>
          <div><ShieldCheck size={11} /><span>INTEGRITY</span><b>99.98%</b></div>
        </div>
      </aside>

      <div className="archos-spatial-search archos-surface">
        <Search size={14} />
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Locate entity, district or emirate" aria-label="Locate entity, district or emirate" />
        <kbd>⌘K</kbd>
        {query && (
          <div className="archos-spatial-search-results">
            {filteredLandmarks.slice(0, 5).map((landmark) => (
              <button key={landmark.id} type="button" onClick={() => { setSelectedLandmarkId(landmark.id); setQuery(''); }}>
                <span>{landmark.name}</span><small>{landmark.emirate} · {landmark.district}</small>
              </button>
            ))}
      ...
      {showFabric && (
        <div className="archos-spatial-fabric archos-surface">
          <button className="archos-spatial-fabric-close" type="button" onClick={() => setShowFabric(false)} aria-label="Close intelligence fabric"><X size={13} /></button>
      ...
      <div className="archos-spatial-command archos-command-surface">
        <Command size={13} />
        <input value={command} onChange={(event) => setCommand(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') executeCommand(); }} placeholder="Command ArchOS..." aria-label="Command ArchOS" />
        <span className="archos-spatial-command-state">{commandCount ? `${commandCount} EXECUTED` : lastCommand}</span>
        <button type="button" onClick={executeCommand}>EXECUTE</button>
      </div>

      <div className="archos-spatial-footer">
        <span>ARCHOS SPATIAL RUNTIME</span><i /><span>WORLD MODEL</span><i /><span>AGENT FABRIC</span><i /><span>VERIFICATION</span><i /><span>SIMULATION READY</span>
      </div>
